...
switch type
    ...
    case 'L4'
        ...
        %:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
        if size(coord,2) < 1
            disp('Error two coordinates needed for the L4 element')
        else
            xi=coord(1);
            N=[(xi+1/3)*(xi-1/3)*(xi-1)/(-16/9);  
                (xi+1)*(xi+1/3)*(xi-1/3)/(16/9);   
                (xi+1)*(xi-1/3)*(xi-1)/(16/27);
                (xi+1)*(xi+1/3)*(xi-1)/(-16/27)];
            dNdxi=[(-(9*(xi-1)*(xi-1/3))/16-(9*(xi-1)*(xi+1/3))/16-(9*(xi-1/3)*(xi+1/3))/16);   %dN1
                ((9*(xi+1)*(xi-1/3))/16+(9*(xi+1)*(xi+1/3))/16+(9*(xi-1/3)*(xi+1/3))/16);    %dN4
                ((27*(xi-1)*(xi+1))/16+(27*(xi-1)*(xi-1/3))/16 +(27*(xi+1)*(xi-1/3))/16);    %dN2
                (-(27*(xi-1)*(xi+1))/16-(27*(xi-1)*(xi+1/3))/16-(27*(xi+1)*(xi+1/3))/16)];    %dN3
        end
